...
FMApp.config(function($interpolateProvider){
...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    FM.projectId = function(){
      return document.getElementById('projects').value;
    };
    $scope.triggerHashChange= function(){
    ...
    var hash2paths = function (relPath) {
      ...
      var path = '#/';
      paths.push({name: FM.projectId(), path: path});
    ...
    setCurFiles = function (relPath) {
      
      console.log('Rel Path', relPath);
     
      $http.get('/admin/api' + relPath+'?project='+FM.projectId())
    ...
    var downloadFile = function (file) {
      window.open('proects/'+ FM.projectId()+'/'+ file.relPath);
    };
    ...
    FM.download = function () {
      ...
      for (var i in FM.selection) {
        link.setAttribute('href', 'projects/'+FM.projectId()+'/'+ FM.selection[i].relPath);
    ...
    FM.delete = function () {
      for (var i in FM.selection) {
        ...
        var url = 'admin/api' + relPath;
        httpRequest('DELETE', url, null, null);
      }
    };

    FM.move = function (target) {
      var url = 'admin/api' + encodeURI(target);
    ...
    /* FM.archive = function (archive) {
      ...
      var url = 'admin/api' + FM.curFolderPath + encodeURI(archive);
      var src = FM.selection.map(function (file) {
        return file.relPath;
      });
    ...
    FM.rename = function (newName) {
      var url = 'admin/api' + FM.selection[0].relPath;
    ...
    FM.createFolder = function (folderName) {
      var url = 'admin/api' + FM.curFolderPath + encodeURI(folderName);
    ...
    FM.upload = function () {
      ...
      $.each(FM.uploadFile,function(index,elem){
        console.log('upload file ', index);
        formData.append('upload_'+index, elem);
      });

      //formData.append('upload', FM.uploadFile);
      var url = 'admin/api' + FM.curFolderPath ;//+ encodeURI(FM.uploadFile.name);
    ...
    FM.btnDisabled = function (btnName) {
      switch (btnName) {
        ...
        return (FM.selection.length === 0) ? true : false;
        case 'move':
        case 'archive':
